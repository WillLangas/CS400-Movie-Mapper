...
public class TestFrontend {
  ...
  public void runTests() {
    ...
    System.out.print(
        "Test 'r' rating mode (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
    if (this.testFrontEndForRatings()) {
      System.out.println("PASSED");
    } else {
      System.out.println("FAILED");
    }
    ...
    System.out.print(
        "Test 'r' and '5' for rating selection (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
    if (this.testFrontEndRatingSelected()) {
      System.out.println("PASSED");
    } else {
      System.out.println("FAILED");
    }
  ...
  public boolean testFrontendGForGenres() {
    ...
    try {
      ...
      if (frontend != null && appOutput.contains("Horror") && appOutput.contains("Action")
          && appOutput.contains("Comedy") && appOutput.contains("Musical")
  ...
  /**
   * This test checks to ensure the ratings selection mode works properly. It enters 'r' to go to
   * ratings mode, then 'x' twice to exit the program. Checks to ensure all integers 0-10 are
   * returned on the ratings screen. Returns true if they are, false otherwise.
   * 
   * @return true if all integers are present, false otherwise
   **/
  public boolean testFrontEndForRatings() {
    PrintStream standardOut = System.out;
    InputStream standardIn = System.in;
    try {
      ...
      // add all tests to this method
      String appOutput = outputStreamCaptor.toString();
      if (frontend != null && appOutput.contains("0") && appOutput.contains("1")
          && appOutput.contains("2") && appOutput.contains("3") && appOutput.contains("4")
          && appOutput.contains("5") && appOutput.contains("6") && appOutput.contains("7")
          && appOutput.contains("8") && appOutput.contains("9") && appOutput.contains("10")) {
        // test passes if all genres from the data are displayed on the screen
        return true;
      } else {
        // test failed
  ...
  /**
   * This test will check to see if the movies are filtered by rating when a number is entered.
   * Returns true if it shows correct movies and false otherwise
   * 
   * @return true if test passes, false otherwise
   **/
  public boolean testFrontEndRatingSelected() {
    PrintStream standardOut = System.out;
    InputStream standardIn = System.in;
    try {
      ...
      // add all tests to this method
      String appOutput = outputStreamCaptor.toString();
      if (frontend != null && !appOutput.contains("Valley Girl")) {
        // test passes if all genres from the data are displayed on the screen
        return true;
      } else {
        // test failed
