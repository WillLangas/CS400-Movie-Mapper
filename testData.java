...
public class testData {
    ...
    public void runTests() {
        ...
        if (this.testGetYear()) {
            System.out.println("Test get year: PASSED");
        } else {
            System.out.println("Test get year: FAILED");
        }
    }
     ...
     * source data in any order. It fails if the list returned is missing one or more titles or if
    ...
    public boolean testReaderMovieTitles() {
        ...
        equalOne = equalOne && (title1.equals(movieList.get(0).getTitle()) || title2
            .equals(movieList.get(0).getTitle()) || title3.equals(movieList.get(0).getTitle()));
        // check if second movie is has of the above titles
        equalOne = equalOne && (title1.equals(movieList.get(1).getTitle()) || title2
            .equals(movieList.get(1).getTitle()) || title3.equals(movieList.get(1).getTitle()));
        // check if third movie is has of the above titles
        equalOne = equalOne && (title1.equals(movieList.get(2).getTitle()) || title2
            .equals(movieList.get(2).getTitle()) || title3.equals(movieList.get(2).getTitle()));
        // true if the three movies have the right titles
        return equalOne;
    }

    /**
     * This test reads in 3 movies, then sorts them. It then checks whether the default sorting
     * order is descending based on the average ratings.
     *
     * @return true if the test passed, false if it failed
     */
    ...
    /**
     * This test reads in 3 movies then checks to make sure the years are correct
     *
     * @return true if test passes, false if it failed
     */
    public boolean testGetYear() {
        List<MovieInterface> movieList;
        try {
            movieList = readerToTest.readDataSet(new StringReader(
                "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
            ...
            int[] yearList = {2020, 2020, 2020};
            for (int i = 0; i < yearList.length; ++i) {
                if (yearList[i] != movieList.get(i).getYear()) {
                    return false;
                }
            }
        } catch (Exception e) {
    ...
    /**
     * This test reads in three movies and checks that the getDirector() method is returning the
     * correct director for each film
     *
     * @return true if test passes, false if failure
     */
        ...
        try {
            movieList = readerToTest.readDataSet(new StringReader(
                "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
                    ...
                    + "Valley Girl,Valley Girl,2020,\"Comedy, Musical, Romance\",102,USA,English,Rachel Lee Goldenberg,\"Amy Talkington, Andrew Lane\",Sneak Preview Productions,\"Jessica Rothe, Josh Whitehouse, Jessie Ennis, Ashleigh Murray, Chloe Bennet, Logan Paul, Mae Whitman, Mario Revolori, Rob Huebel, Judy Greer, Alex Lewis, Alex MacNicoll, Danny Ramirez, Andrew Kai, Allyn Rachel\",\"Set to a new wave '80s soundtrack, a pair of young lovers from different backgrounds defy their parents and friends to stay together. A musical adaptation of the 1983 film.\",5.4\n"));
            String[] directorList =
                {"Ryan Bury, Jennifer Bonior", "Rene Perez", "Rachel Lee Goldenberg"};

            for (int i = 0; i < directorList.length; ++i) {
                if (!movieList.get(i).getDirector().equals(directorList[i])) {
                    System.out.println("IS: " + movieList.get(i).getDirector());
                    System.out.println("SHOULD BE: " + directorList[i]);
                    System.out.println("testGetDirector failed for index " + i);
                    return false;
                }
            }
        } catch (Exception e) {
